...
class GNN(nn.Module):
    def __init__(self,
        ...
        #     #     self.out[t] = nn.Linear(hidden_dim, out_dim)
            # if t in ["disease_pred"] :
            #     self.out[t] = nn.Linear(hidden_dim, 9) #질병 예측 9개
    ...
    def encode(self, g, h, causal: bool = False):
        """Return node embeddings without task-specific heads."""
        z = self.get_logit(g, h, causal)
        return z
